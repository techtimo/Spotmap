...
function Row( { label, value } ) {
    return (
        <tr>
            <td style={ { paddingRight: '1.5rem', color: '#555', whiteSpace: 'nowrap', verticalAlign: 'top' } }>
                { label }
            </td>
...
export default function FeedStatsModal( { feed, onClose } ) {
    ...
    return (
        ...
        >
            { error && (
                <p style={ { color: '#cc1818' } }>Error: { error }</p>
            ) }
            { ! error && ! stats && <Spinner /> }
            { stats && (
                <table style={ { borderCollapse: 'collapse', width: '100%' } }>
                    <tbody>
                        ...
                        { stats.busiest_day_count > 1 && (
                            <Row
                                label="Busiest day"
                                value={ `${ formatDate( stats.busiest_day_date ) } (${ stats.busiest_day_count } points)` }
                            />
                        ) }
                        <Row
                            ...
                            value={
                                stats.max_distance_day_date
                                    ? `${ formatDate( stats.max_distance_day_date ) } (${ stats.max_distance_day_km } km)`
                                    : null
                            }
                        />
                        { stats.avg_altitude !== null && (
                            <Row
                                label="Avg. altitude"
                                value={ `${ stats.avg_altitude.toLocaleString( 'en' ) } m` }
                            />
                        ) }
                        { stats.created_at && (
                            <Row
