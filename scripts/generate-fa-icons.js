...
const fs = require('fs');
const icons = yaml.load(fs.readFileSync('node_modules/@fortawesome/fontawesome-free/metadata/icons.yml', 'utf8'));
...
const out = [
  ...
  ...solid.reduce((rows, _, i) => {
    if (i % 8 === 0) rows.push('	' + solid.slice(i, i+8).map(x => JSON.stringify(x)).join(', ') + ',');
    return rows;
  }, []),
  '];
',
