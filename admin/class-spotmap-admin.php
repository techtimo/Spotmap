...
class Spotmap_Admin
{
	...
	static public function add_cron_schedule($schedules)
	{
	...
	{
		add_options_page('Spotmap Options', 'Spotmap 🗺', 'manage_options', 'spotmap', ['Spotmap_admin', 'display_options_page']);
	}
	...
	public static function register_settings()
	{
		...
		foreach (get_option("spotmap_api_providers") as $provider => $name) {
			...
			register_setting('spotmap-feed-group', 'spotmap_' . $provider . '_name', ['sanitize_callback' => ['Spotmap_admin', 'validate_feed_name']]);
			register_setting('spotmap-feed-group', 'spotmap_' . $provider . '_id', ['sanitize_callback' => ['Spotmap_admin', 'validate_feed_id']]);
			...
			add_settings_section(
				$provider . '-feeds',
				$name,
				['Spotmap_admin', 'settings_section_' . $provider],
				'spotmap-feed-group'
			);
			for ($i = 0; $i < $count; $i++) {
				$settings = [
				...
				foreach ($settings as $key => $value) {
					...
					add_settings_field(
						...
						$value["label"],
						['Spotmap_admin', 'generate_settings'],
						'spotmap-feed-group',
						'findmespot-feeds',
						[
		...
		add_settings_section(
			...
			['Spotmap_admin', 'settings_section_marker'],
		...
		self::add_bulk_settings($settings, 'spotmap-marker-group', 'spotmap-marker-shape-section');

		// MARKER ICON SECTION
		add_settings_section(
			'spotmap-marker-icon-section',
			__('Marker icon options'),
			['Spotmap_admin', 'settings_section_icons'],
			'spotmap-marker-group',
		);
		// error_log(print_r(get_option('spotmap_marker'),TRUE));
		...
		self::add_bulk_settings($settings, 'spotmap-marker-group', 'spotmap-marker-icon-section');
		...
		add_settings_section(
			...
			['Spotmap_admin', 'settings_section_messages'],
			'spotmap-marker-group'
		);
		$settings = [];
		foreach ($point_types as $point_type) {
		...
		self::add_bulk_settings($settings, 'spotmap-marker-group', 'spotmap-marker-custom-message-section');
		...
		add_settings_section(
			...
			['Spotmap_admin', 'settings_section_thirdparty'],
		...
		self::add_bulk_settings($settings, 'spotmap-thirdparties-group', 'spotmap-thirdparty-section');

		// DEFAULT SECTION
		register_setting('settings_section_defaults', 'spotmap_default_values');
		add_settings_section(
			'spotmap-defaults',
			__('Default Values'),
			['Spotmap_admin', 'settings_section_defaults'],
		...
		self::add_bulk_settings($settings, 'spotmap-defaults-group', 'spotmap-defaults');
	}


	public static function add_bulk_settings($settings, $settings_group, $settings_section)
	{
	...
	static function generate_settings($args)
	{
		// error_log(print_r($args,TRUE));
		foreach (['text', 'password', 'number',] as $type) {
			if ($args['type'] == $type) {
				self::generate_text_field($args);
			}
		}
		if ('textarea' == $args['type']) {
			self::generate_textarea($args);
		}
		if ('dropdown' == $args['type']) {
			self::generate_dropdown_field($args);
		}
		if ('icon' == $args['type']) {
			self::generate_icon_field($args);
		}
	}
	static function generate_textarea($args)
	{
...
?>
	...
	static function generate_text_field($args)
	{
	...
	static function generate_dropdown_field($args)
	{
	?>
		<select name="<?php echo $args['id'] ?>">
			<?php foreach ($args['options'] as $key) {
				...
				// else if (!isset($args["value"]) && isset($key[1])) {
				// 	$option_value = $key[1];
				// }
...
<?php
	...
	static function settings_section_findmespot($args)
	{
	...
	static function settings_section_messages($args)
	{
	...
	static function settings_section_icons($args)
	{
	...
	static function settings_section_marker($args)
	{
		echo '<p id=' . $args['id'] . '>' . __("Change the marker size for each message type.") . '<br>
		</p>';
	}
	static function settings_section_thirdparty($args)
	{
	...
	static function settings_section_defaults($args)
	{
	...
	static function validate_feed_name($new_feed_name)
	{
		foreach ($new_feed_name as $index => &$feed_name) {
			$feed_name = sanitize_text_field($feed_name);
			$old_feed_name = get_option("spotmap_findmespot_name") ? get_option("spotmap_findmespot_name")[$index] : "";
			if (empty($feed_name)) {
				continue;
			...
			$result = self::$db->rename_feed_name($old_feed_name, $feed_name);
		}
		return $new_feed_name;
	}

	static function validate_feed_id($new_feed_id)
	{
	...
	static function allow_gpx_upload($mime_types)
	{
		$mime_types['gpx'] = 'text/xml';
		return $mime_types;
	}
	static function add_link_plugin_overview($links)
	{
		$mylinks = [
	...
	static function get_local_timezone()
	{
		global $wpdb;
	...
	static function get_maps_config_content($section)
	{
	...
	{
		return self::get_maps_config_content("overlays");
	}

	public static function get_maps()
	{
		$maps = self::get_maps_config_content("baseLayers");
	...
	public function delete_images_from_map($attachment_id)
	{
		...
		// if(self::$db->does_media_exist($attachment_id)){
		self::$db->delete_media_point($attachment_id);
		// }
	}

	// This static functions get's called when new media is uploaded and adds it to the spotmap table if GPS info is part of EXIF data
	public static function add_images_to_map($attachment_id)
	{
